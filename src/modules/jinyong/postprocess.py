...
def _write_export_readme(output_dir: Path, manifest: dict) -> None:
    lines = [
        "# 金庸图谱数据包 v1",
        "",
        "这是面向使用者和下游应用的金庸图谱成果包。普通查询、文学分析和应用原型都应该从这里开始，而不是从 `runs/`、`docs/research/` 或 `docs/plans/` 开始。",
        "",
        "你可以把它理解成两层中的“使用层”：",
        "",
        "- 使用层：`artifacts/jinyong-v1`，提供相对干净、稳定、可直接读取的数据。",
        "- 分析层：`runs/`、`docs/research/`、`docs/plans/`，保留建图、实验、成本、后处理和复盘记录。",
        "",
        "分析层没有被废弃，只是从日常使用入口退到档案和维护入口。做查询时不需要先读那些过程文档。",
        "",
        "## 适合做什么",
        "",
        ...
        "",
        "```text",
        "请基于 artifacts/jinyong-v1 金庸图谱数据包回答问题。回答要面向普通读者，不要解释工程过程。请区分：图谱直接支持、基于证据的推断、证据不足处。",
        "问题：……",
        "```",
        "",
        "这和普通 AI 闲聊的区别是：回答应先检索图谱中的人物、关系和全局索引，再组织分析；不能只凭模型记忆讲金庸。",
        "",
        "## 当前规模",
        "",
...
把下面这段话交给其他 AI agent，然后追加你的问题：

```text
请基于 artifacts/jinyong-v1 金庸图谱数据包回答问题。先读取 README.md 和 manifest.json，再按需要读取 works/<作品>/graph.json、global/people.json、global/crosswork_people.json。

回答要让普通读者能懂，不要解释工程过程。请区分：
1. 图谱直接支持的证据
2. 基于证据的推断
3. 证据不足或需要原文复核的地方
```
...
```text
请基于《鹿鼎记》的图谱，分析韦小宝如何同时连接皇权、天地会、神龙教和吴三桂势力。
```
...
```text
请比较《碧血剑》《书剑恩仇录》《鹿鼎记》中“反清”主题的差异：个人选择、组织纪律和历史现实分别占多大比重？
```

```text
请找出“师承关系变成价值冲突”的典型例子，例如令狐冲与岳不群、杨过与郭靖/黄蓉体系、张无忌与明教/武当之间的关系。
```
...
这种问题太大，图谱只能提供局部证据，容易变成模型自由发挥。

```text
谁是金庸宇宙战斗力第一？
```

图谱记录人物、关系和描述，不是严谨战力排行榜。除非先限定作品、武功、对手和证据口径。

```text
请证明某某人物一定影射现实中的某某人。
```

图谱不包含可靠的作者意图证据，只能说明文本内部关系。
...
def _write_sample_answers(output_dir: Path) -> None:
    content = """# 金庸图谱可读分析样例

这些样例展示的是“怎么把图谱证据写成普通读者能懂的分析”。它们不是最终论文结论，而是验证成果包是否好用的参考答案。

## 样例 1：《笑傲江湖》里令狐冲与岳不群为什么不是简单的师徒决裂？

一句话结论：  
令狐冲与岳不群的关系不是普通的“徒弟背叛师父”，而是师门伦理、个人自由和权力算计逐步撕裂的结果。

关键证据：

1. 图谱中存在 `令狐冲 -> 岳不群` 的 `传授` 关系，描述里同时包含“自幼被收录门下”“受抚养、教导武功与门规”“感恩与敬重”。这说明二人的关系起点不是敌对，而是长期师徒和类父子关系。
2. 同一关系描述后半部分转向冲突：岳不群对令狐冲的怀疑、排斥和政治化利用，使这条师徒关系从伦理关系变成权力关系。
3. 图谱还显示 `令狐冲 -> 风清扬` 的 `传授` 关系，核心是独孤九剑和“以无招胜有招”。这给令狐冲提供了另一套武学和精神秩序。
4. `令狐冲 -> 任我行` 的关系显示他又被卷入吸星大法和日月神教权力结构；`令狐冲 -> 任盈盈` 则显示他的情感归属逐渐离开华山派伦理。

图谱直接支持：  
令狐冲身上同时存在华山师承、风清扬剑道、任我行武学遗产、任盈盈情感网络四条线。

基于证据的推断：  
岳不群和令狐冲的破裂，不只是误会或背叛，而是两套价值系统的冲突：岳不群代表门派名分和权力目标，令狐冲逐渐转向自由、情义和个人选择。

不确定处：  
图谱能说明关系结构和方向，但不能替代对具体章节语气、心理描写的细读。

## 样例 2：《天龙八部》三主角的武学路径有什么不同？

一句话结论：  
萧峰的武功绑定身份悲剧，段誉的武功来自偶然和天性，虚竹的武功则来自被动继承和命运错位。

关键证据：

1. `萧峰` 的实体描述强调他曾为丐帮帮主、契丹后裔、中原武林英雄，图谱中还有 `萧峰 -> 丐帮`、`萧峰 -> 辽国/大辽` 等关系。这说明他的武功和身份冲突纠缠在一起。
2. `萧峰 -> 大宋`、`萧峰 -> 南征` 等关系显示，他的武力最终不是用来争胜，而是面对国家和族群冲突。
3. `段誉` 的实体描述显示他最初厌恶武学，却因无量山机缘得到北冥神功和凌波微步；图谱中也有 `段誉 -> 北冥神功`、`段誉 -> 凌波微步`、`段誉 -> 六脉神剑` 等 `使用` 关系。
4. `虚竹` 的实体描述显示他原是少林小僧，因珍珑棋局被无崖子传功，成为逍遥派掌门。其武学不是主动追求，而是命运强加。

图谱直接支持：  
三人的武学都不是单纯“练成绝技”：萧峰连着帮派和国家，段誉连着奇遇和皇族身份，虚竹连着佛门戒律和逍遥派继承。

基于证据的推断：  
《天龙八部》不是在写三种升级路线，而是在写三种“力量与身份不匹配”的人生。

不确定处：  
图谱能抓到主干关系，但对武学招式细节、章节先后和心理转折仍需要原文校验。

## 样例 3：郭靖、杨过、张无忌的跨书出现方式有何不同？

一句话结论：  
郭靖和杨过在跨书中更像历史记忆和武林谱系节点，张无忌的跨书出现更弱，更接近零散引用。

关键证据：

1. 全局人物层显示 `郭靖` 出现在 5 部作品，主场推定为《射雕英雄传》，最高度数为 727，核心邻居包括朱聪、丘处机、柯镇恶、江南七侠等。
2. `杨过` 出现在 4 部作品，主场推定为《神雕侠侣》，最高度数为 683，核心邻居包括郭襄、雕、黑龙潭、瑛姑、一灯。
3. `张无忌` 出现在 2 部作品，主场推定为《倚天屠龙记》，最高度数为 690，但非主场只在《飞狐外传》中弱出现。
4. 跨书候选层显示 `郭靖` 在《笑傲江湖》中是疑似污染或弱证据，`杨过` 属于待复核共享引用，`张无忌` 也是待复核。

图谱直接支持：  
三人在各自主场都有高连接度，但非主场出现强度差异很大。

基于证据的推断：  
郭靖和杨过因为处在射雕三部曲的连续谱系里，更容易成为后续武林记忆的一部分；张无忌虽是主角，但跨书传播性较弱。

不确定处：  
跨书候选包含污染风险，不能把所有非主场出现都当成作者有意的宇宙联动。

## 样例 4：《鹿鼎记》中韦小宝如何同时连接朝廷、江湖和反清组织？

一句话结论：  
韦小宝不是传统武侠主角，而是一个在多套权力系统之间滑行的人。

关键证据：

1. 图谱中 `韦小宝 -> 康熙` 的关系描述包含主仆、朋友、师徒、君臣等多重身份，说明他进入权力中心不是靠武功，而是靠亲密关系和机变。
2. `韦小宝 -> 天地会` 的关系显示他加入反清复明组织，并成为青木堂香主，说明他同时又在反清组织内部拥有身份。
3. `吴三桂 -> 韦小宝` 的关系显示两者是敌对和监视关系，韦小宝代表康熙调查、对抗吴三桂。
4. `韦小宝 -> 神龙教` 的关系显示他也被卷入邪教式江湖组织，并通过假冒、编造和周旋脱身。

图谱直接支持：  
韦小宝同时连接康熙、天地会、吴三桂、神龙教等多个系统。

基于证据的推断：  
《鹿鼎记》的主角能力不在武学，而在跨系统生存：他能在皇权、帮会、反清组织和江湖邪教之间转换身份。

不确定处：  
图谱能显示他连接了哪些系统，但对讽刺意味、语言风格和喜剧效果的解释仍需要读原文。

## 样例 5：《碧血剑》《书剑恩仇录》《鹿鼎记》的反清主题有什么差异？

一句话结论：  
三部作品都写反清，但重点不同：《碧血剑》偏向乱世中的个人选择，《书剑恩仇录》偏向组织化行动，《鹿鼎记》则把反清理想放进现实权力和人情网络中反讽。

关键证据：

1. 《碧血剑》图谱中 `袁承志` 连接华山派、金蛇郎君遗产、闯军、阿九、温青青等节点，说明他的行动常在家国、师承、情感和复仇之间摇摆。
2. 《书剑恩仇录》图谱中 `陈家洛`、`文泰来`、`红花会`、`乾隆` 等关系密集，反清更明显表现为组织行动、营救、结盟和政治秘密。
3. 《鹿鼎记》图谱中 `韦小宝` 同时连接 `康熙`、`天地会`、`陈近南`、`吴三桂`、`神龙教`，说明反清不再是单纯立场，而被放入多重身份和利益周旋中。
4. `韦小宝 -> 天地会` 与 `韦小宝 -> 康熙` 同时存在强关系，构成了《鹿鼎记》最核心的张力：反清组织身份和皇帝亲信身份并存。

图谱直接支持：  
三部作品中反清相关节点的组织形态不同：袁承志更像个人行动中心，陈家洛更像组织领袖，韦小宝更像多个权力系统之间的中介。

基于证据的推断：  
金庸对“反清”的处理不是固定模板，而是从英雄叙事、组织叙事逐渐转向现实主义和反讽。

不确定处：  
图谱能支持结构比较，但对历史观、民族观和作者态度的判断，需要结合具体版本、章节和写作年代。

## 样例 6：少林、武当、丐帮在不同作品中更像什么？

一句话结论：  
少林常像武林秩序和道德权威，武当常像师承与正统谱系，丐帮则更像连接江湖、民间和政治局势的流动组织。

关键证据：

1. 在多部作品图谱中，`少林` 往往连接高僧、戒律、寺院、武功和武林大会等节点，常承担裁判、收容、冲突调停或权威背书的功能。
2. `武当` 在《倚天屠龙记》等作品中与张三丰、张无忌、俞岱岩、宋远桥等人物相关，关系更集中在师承、门派伦理和正派谱系。
3. `丐帮` 在《射雕英雄传》《天龙八部》等作品中连接帮主、帮众、江湖行动和政治冲突，组织边界比寺院和门派更流动。
4. 萧峰、洪七公、黄蓉等人物都和丐帮形成高价值关系，说明丐帮经常把个人英雄和集体组织连接起来。

图谱直接支持：  
少林、武当、丐帮在多个作品中都有跨人物、跨事件的组织连接，但连接对象和叙事功能不同。

基于证据的推断：  
三类组织不是简单的“门派列表”。它们分别承担秩序、谱系和社会连接三类叙事功能。

不确定处：  
当前成果包还没有单独抽象出“组织层”，所以这种分析需要从图谱中的实体和关系归纳，仍有进一步后处理空间。

## 样例 7：为什么说图谱回答不能等同于普通 AI 记忆回答？

一句话结论：  
图谱的价值不是让 AI “更会聊金庸”，而是迫使回答先落到可检查的人物、关系和全局索引上。

关键证据：

1. 单书 `graph.json` 提供实体、关系、关系描述和权重，能检查某个结论到底连接了哪些人物或事物。
2. `global/people.json` 提供人物在多部作品中的出现位置、主场作品推定、连接度和邻居样例，适合做跨书比较。
3. `global/crosswork_people.json` 明确标出跨书候选是连续作品共享、疑似污染，还是需要复核，不鼓励把所有跨书出现都当成事实。
4. `global/noise_summary.json` 保留噪声候选摘要，提醒使用者哪些结果可能需要谨慎。

图谱直接支持：  
回答可以追溯到具体数据文件，而不是只依赖模型训练记忆。

基于证据的推断：  
这会减少幻觉，但不能消灭幻觉。更准确的说法是：图谱把自由发挥变成了有证据边界的分析。

不确定处：  
图谱本身来自模型抽取，仍可能有漏抽、错抽、关系类型粗糙和描述过长的问题。重要结论仍应回到原文复核。
"""
    examples_dir = output_dir / "examples"
    examples_dir.mkdir(parents=True, exist_ok=True)
    (examples_dir / "sample_answers.md").write_text(content, encoding="utf-8")
...
def run_export_corpus(
    ...
    _write_sample_answers(output_dir)
    ...
    return {
        ...
        "sample_answers": str(output_dir / "examples" / "sample_answers.md"),
